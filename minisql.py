...
try :
                ...
                if str(ic).lower() != "or" and str(ic).lower() != "and":
                    ...
                    if bool(num_format.match(str(operands[-1]))): # if the second operand in a number
                        ...
                        for i in final_answer:
                            temp = []
                            if ((op==4 and int(i[ind])>int(operands[1]))or(op==3 and int(i[ind])<int(operands[1]))or(op==2 and int(i[ind])<=int(operands[1]))or(op==1 and int(i[ind])>=int(operands[1]))or(op==5 and int(i[ind])==int(operands[1]))):
                                for j in i:
                                    temp.append(j)
                                currop.append(temp)
    ...
    def aggquery(select_part,from_part):
        ...
        if(str(select_part[0][0:7]).lower()=="average"):
            ...
            for i in opentables[0]:
                sum+= int(i[index])
            return float(sum)/len(opentables[0])
    ...
    print "Error Occoured Try Again"
